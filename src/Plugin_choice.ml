...
let supported_separators =
  [ "|"; "||"; "&"; "&&"; "vs"; "or"; "and"; "ou"; "et"; "contre"; "versus" ]

(* Gives an arbitrary choice based on a arbitrary hash: gives a total order *)
let determinate_choice l =
  let val_of_str a =
    Hashtbl.hash (a |> String.trim |> String.lowercase_ascii)
  in
...
let cmds_choice : Command.t list =
  ...
  let cmd_function choice_function result_message failure_message _ str =
    let choice_opt =
      String.trim str
      |> extract_choices supported_separators
      |> Option.map choice_function
    in
    let msg =
  ...
  [
    command_make_choice "choix" "Faire un choix entre deux options" random_l
      "Et c'est décidé ! Ce sera "
      "Malheureusement, il n'est pas possible de parser votre indécision, même \
       avec les dernières percées de nos IA multi-cloud-ready bare metal. \
       Faites un effort !";
    command_make_choice "choice" "Make a choice between two options" random_l
      "And it's decided! It's gonna be "
      "Sorry, unable to parse your query here. Please consider upgrading your \
       bot to a modern language like golang or ruby ;). Who still uses OCaml \
       nowadays?";
    command_make_choice "battle" "Qui que c'est le plus fort ?"
      determinate_choice "Évidemment, ça va être "
      "Pas facile de dire… Z'êtes sûrs de la question ?";
  ]
